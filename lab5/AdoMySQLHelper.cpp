...
CMySQL_Statu CAdoMySQLHelper::MYSQL_net_Insert(const CString& uid,const CString& time){
	if(!this->connect_state){
		CMySQL_Statu m_statu = this->MYSQL_Connect();
		if(m_statu.getType()){
			return m_statu;
	...
	try{
		HRESULT hr = m_pCommand.CreateInstance("ADODB.Command");
		if(FAILED(hr)){
			CMySQL_Statu mysql_statu(5,"创建_CommandPtr智能指针失败");
			return mysql_statu;
		}
		//定义为无参数
		_variant_t vNULL;
		...
		//insert into table_name(字段1, 字段2...) values(字段1值, 字段2值...);
		//m_pCommand->CommandText = "delete from " + (_bstr_t)table + " where UID=\'" + (_bstr_t)uid +"\'";
		m_pCommand->CommandText = "insert into card_net(uid,time) values(\'"+(_bstr_t)uid+"\',\'"+(_bstr_t)time+"\');";
		//非常关键的一句，将建立的连接与命令对象绑定							
		m_pCommand->ActiveConnection = this->m_pConn;
		...
		CString msg = (LPCSTR)e.Description();
		CMySQL_Statu mysql_statu(5,msg);
		return mysql_statu;
	}
	CMySQL_Statu mysql_statu(0,"插入成功");
	return mysql_statu;
...
CMySQL_Statu CAdoMySQLHelper::MYSQL_net_Update(const CString& uid,const CString& time){
	if(!this->connect_state){
		CMySQL_Statu m_statu = this->MYSQL_Connect();
		if(m_statu.getType()){
			return m_statu;
	...
	try{
		HRESULT hr = m_pRecordset.CreateInstance("ADODB.Recordset");
		if(FAILED(hr)){
			CMySQL_Statu mysql_statu(6,"创建recordset指针失败，请检查!");
			return mysql_statu;
		}
		_variant_t table_name = "card_net"; 
		...
		if(!m_pRecordset->ADOEOF){
			...
			while(!m_pRecordset->ADOEOF){
				...
				if(uid == str){
					...
					//方式三
					_variant_t var3 = (LPCTSTR)time;
					m_pRecordset->PutCollect("time", var3);
					flag = true;
					//必须在移动游标前执行更新!!
					m_pRecordset->Update();
					break;
			...
			m_pRecordset=NULL;
			if(!flag){
				CMySQL_Statu mysql_statu(6,"无该uid对应的值");
				return mysql_statu;
			}
		}else{
			CMySQL_Statu mysql_statu(6,"无该uid对应的值");
		...
		CString msg = (LPCSTR)e.Description();
		CMySQL_Statu mysql_statu(6,msg);
		return mysql_statu;
	}
	CMySQL_Statu mysql_statu(0,"修改成功");
	return mysql_statu;
...
CMySQL_Statu CAdoMySQLHelper::MYSQL_net_Query(const CString& uid,CString &time){
	if(!this->connect_state){
		CMySQL_Statu m_statu = this->MYSQL_Connect();
		if(m_statu.getType()){
			return m_statu;
	...
	try{
		HRESULT hr = m_pRecordset.CreateInstance("ADODB.Recordset");
		if(FAILED(hr)){
			CMySQL_Statu mysql_statu(7,"创建recordset指针失败，请检查!");
			return mysql_statu;
		}

		CString strsql = "select * from card_net where uid = \'"+uid+"\';";
		_variant_t sql=(LPCTSTR)strsql; 
		...
		if(!m_pRecordset->ADOEOF){
			...
			while(!m_pRecordset->ADOEOF){
				...
				if(flag){
					...
					CMySQL_Statu mysql_statu(7,"出现重复字段，请检查!");
					return mysql_statu;
				}else{
					flag = true;
				}	
				_variant_t var1 = m_pRecordset->Fields->GetItem("time")->Value;
				//取int类型
				//var1.ChangeType(VT_INT);
				//int varInt = var1.intVal;
			...
			if(!flag){
				CMySQL_Statu mysql_statu(7,"无该uid对应时间，请重新读取卡片!");
				return mysql_statu;
			}
		...
		CString msg = (LPCSTR)e.Description();
		CMySQL_Statu mysql_statu(7,msg);
		return mysql_statu;
	}
	CMySQL_Statu mysql_statu(0,"查询成功");
	return mysql_statu;
